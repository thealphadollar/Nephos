...
class TestScheduler(TestCase):
    ...
    def test_init_utc(self):
        with mock.patch('nephos.scheduler.LOG') as mock_log:
            Scheduler()
    ...
    def test_add_recording_job(self):
        with mock.patch('nephos.scheduler.Scheduler') as mock_scheduler, \
                mock.patch('nephos.scheduler.LOG') as mock_log:
    ...
    def test_add_maintenance_jobs(self):
        with mock.patch('nephos.scheduler.Scheduler') as mock_scheduler, \
                mock.patch('nephos.scheduler.LOG') as mock_log:
    ...
    def test_print_jobs(self, mock_output):
        Scheduler().print_jobs()
        output = mock_output.getvalue()
        expected = "Pending jobs:"
        self.assertIn(expected, output)

    @mock.patch('nephos.scheduler.input', return_value=mock_job_id)
    def test_rm_recording_job(self, mock_input):
        with mock.patch('nephos.scheduler.LOG') as mock_log, \
                mock.patch('nephos.scheduler.Scheduler') as mock_scheduler:
    ...
    @mock.patch('nephos.scheduler.input', return_value=mock_job_id)
    def test_rm_non_existing_recording_job(self, mock_input):
        with mock.patch('nephos.scheduler.LOG') as mock_log:
